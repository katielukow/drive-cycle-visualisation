...
def current_rate_calc(data_all, dc_all):
    ...
    time_off = (data_all['DateTime'].iloc[-1] - data_all['DateTime'].iloc[0]).total_seconds() - time_discharge - time_charge
    time_data = pd.Series({
    ...
    return I_discharge, I_charge, time_data
...
def app():
    ...
    simple_exp = [
        "Discharge at "+ str(I_discharge) +"C until 3 V",
    ...
    st.write('The following PyBaMM load profile considers the C-Rates from the filtered field data previously presented. We make the assumption that there is a full discharge and charge cycle with the time distribution being within 0.2% of the total time. Taking this in to account, the representative off time is determined and the PyBaMM experiment is defined.')

    discharge_time = 1/I_discharge 
    total_time = np.round(discharge_time / (time_data['Discharge']/time_data.sum(axis=0)),2)
